...
func (j *Job) Check() error {
	...
	if !etcd.IsValidAsKeyPath(j.ID) {
		return error2.ErrIllegalJobId
	}

	j.Name = strings.TrimSpace(j.Name)
	if len(j.Name) == 0 {
		return error2.ErrEmptyJobName
	}
	...
	if !etcd.IsValidAsKeyPath(j.Group) {
		return error2.ErrIllegalJobGroupName
	}
	...
	if len(strings.TrimSpace(j.Command)) == 0 {
		return error2.ErrEmptyJobCommand
	}
...
func (j *Job) Valid() error {
	...
	if !j.validUser() {
		return error2.ErrSecurityInvalidUser
	}

	if !j.validCmd() {
		return error2.ErrSecurityInvalidCmd
	}
